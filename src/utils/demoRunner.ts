...
function section(title: string) {
  console.log('');
  console.log(`  ${bold(blue(`── ${title}`))} ${dim('─'.repeat(Math.max(0, 46 - title.length)))}`);
}
...
const SCENARIO = {
  ...
  taskDescription: 'Integrate the payment gateway API and deliver working TypeScript SDK with tests',
  paymentAmount:   0.5,
  requirements: [
    { id: 'req-001', description: 'TypeScript SDK with full type definitions' },
    { id: 'req-002', description: 'Unit tests with ≥80% coverage' },
    { id: 'req-003', description: 'API integration verified against staging environment' },
  ],
};
  ...
  items: [
    {
      ...
      description: 'Full TypeScript SDK with branded types, generic response wrappers, and JSDoc coverage',
      hash: generateIntegrityHash({ ref: 'src', ts: Date.now() }),
    },
    {
      requirementId: 'req-002',
      evidenceType:  'test_report',
      artifactReference: 'https://github.com/devbot/payment-sdk/actions/runs/1234/artifacts',
    ...
    },
    {
      ...
      description: 'Staging run log: all 12 payment endpoints returning 200, latency p99 < 220 ms',
...
async function runDemo() {
  ...
  log('📦', `Artifacts`, `${EVIDENCE_PAYLOAD.items.length} items, all requirements covered`);
  ...
  if (!verificationResult) {
    log('❌', red('Verification returned null — check agreement/submission data'));
    process.exit(1);
  }
  ...
  log(
    ...
    verificationResult.approved ? green('Verification PASSED') : red('Verification FAILED'),
  );
  log('🔍', `Rule score`, `${verificationResult.ruleScore.toFixed(1)}`);
  log('🤖', `AI judge score`, `${verificationResult.aiScore.toFixed(1)}`);
  log('📊', `Evidence quality`, `${verificationResult.evidenceScore.toFixed(1)}`);
  log('🛡️', `Coverage`, `${verificationResult.evidenceCoverage.toFixed(1)}%`);
  log('🎯', `Confidence`, `${(verificationResult.confidenceScore * 100).toFixed(1)}%`);
  log('🔑', `Integrity hash`, verificationResult.integrityHash.slice(0, 20) + '…');
  divider();
  ...
  try {
    ...
    if (err?.message?.includes('Missing Pharos') || err?.message?.includes('PRIVATE_KEY')) {
      log('⚠️', yellow('Blockchain env vars not set — skipping on-chain step'));
      log('   ', dim('Add PHAROS_RPC_URL / PHAROS_CHAIN_ID / PHAROS_PRIVATE_KEY / PHAROS_ESCROW_CONTRACT to .env'));
      log('   ', dim('Verification result above is real. Settlement would execute once env is configured.'));
    } else {
  ...
  console.log(`  ${bold('Rule score')}     ${verificationResult.ruleScore.toFixed(1)}`);
  console.log(`  ${bold('AI score')}       ${verificationResult.aiScore.toFixed(1)}`);
  console.log(`  ${bold('Confidence')}     ${(verificationResult.confidenceScore * 100).toFixed(1)}%`);
  console.log(`  ${bold('Integrity')}      ${verificationResult.integrityHash.slice(0, 32)}…`);
  divider();
  console.log('');
